...
// Helper function to safely use SweetAlert
function showAlert(options) {
  if (typeof window.Swal !== "undefined") {
    return window.Swal.fire(options)
  } else {
    console.error("SweetAlert2 is not available. Message:", options.text || options.title)
...
// Preview vendor image function
function previewVendorImageSettings(event) {
  const file = event.target.files[0]
  if (file) {
    const reader = new FileReader()
    reader.onload = (e) => {
      const previewDiv = document.getElementById("vendor-image-preview")
      const previewImg = document.getElementById("vendor-image-display")
...
document.getElementById("vendor-image-upload").addEventListener("change", previewVendorImageSettings)
...
function saveVendorInfo() {
  ...
  // Create FormData for file upload
  const formData = new FormData()
  const vendorImageFile = document.getElementById("vendor-image-upload").files[0]

  if (vendorImageFile) {
    formData.append("vendorImage", vendorImageFile)
  }

  // Add other vendor data to FormData
  ...
  xhr.onload = () => {
    if (xhr.status === 200) {
      ...
      // Reload vendor data to show updated image
      loadVendorData()
    } else {
      console.error("Error response:", xhr.responseText)
      showAlert({
  ...
  xhr.onload = () => {
    if (xhr.status === 200) {
      try {
        ...
        // Load vendor image if available
        if (vendorData.vendorImage) {
          const previewDiv = document.getElementById("vendor-image-preview")
          const previewImg = document.getElementById("vendor-image-display")
          if (previewDiv && previewImg) {
            previewImg.src = `/uploads/${vendorData.vendorImage}`
            previewDiv.style.display = "block"
          }
        }
      } catch (error) {
