...
document
  .getElementById("moderator-image-upload")
...
function updateSidebarInfo() {
  document.getElementById("sidebar-shop-name").textContent =
    document.getElementById("shop-name").value;
  document.getElementById("sidebar-shop-description").textContent =
...
function saveModeratorInfo() {
  ...
  xhr.open(
    "PUT",
    `http://localhost:3000/moderator/update/${currentUserId}`,
    true
  );
  xhr.setRequestHeader("Authorization", "Bearer " + getToken());
...
function showSection(event, sectionId) {
  ...
  if (
    (sectionId === "shop-info" || sectionId === "stock-limiter") &&
  ...
  xhr.onload = () => {
    if (xhr.status === 200) {
      try {
        ...
        document.getElementById("sidebar-shop-description").textContent =
          shopData.shopdescription || "Shop Description";
        document.getElementById("sidebar-address").textContent =
          "📍 " + (shopData.adresse || "Address");
        document.getElementById("sidebar-phone").textContent =
          "📞 " + (shopData.shop_phone || "");

        // Update shop logo if available
        if (shopData.shopLogo) {
          document.getElementById("shop-logo").src =
            `/uploads/${shopData.shopLogo}`;
        }
      } catch (error) {
