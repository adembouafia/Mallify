...
function setupRefreshButtons() {
    document.querySelectorAll('.card-tools button[data-bs-toggle="tooltip"]').forEach(button => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let checkApexCharts = function(retries = 0, maxRetries = 3) {
        ...
        console.warn(`ApexCharts not available yet, attempt ${retries+1} of ${maxRetries}...`);
        ...
        if (retries < maxRetries) {
            ...
            setTimeout(() => checkApexCharts(retries + 1, maxRetries), 500 * Math.pow(2, retries));
        } else {
            console.error('ApexCharts library not loaded after multiple attempts! Charts will not work.');
            document.querySelectorAll('#ordersChart, #shopCategoriesChart, #bestSellersChart').forEach(el => {
                el.innerHTML = '<div class="alert alert-danger">Chart library failed to load. Please check your internet connection and refresh the page.</div>';
            });
        }
    };
...
function fetchShopsCount() {
    ...
    // Show loading state
    document.getElementById('totalShops').innerHTML = '<i class="bi bi-hourglass-split"></i>';
    
    xhr.onload = function() {
        if (this.status === 200) {
            try {
                const data = JSON.parse(this.responseText);
                document.getElementById('totalShops').textContent = formatNumber(data.count || 0);
            } catch (e) {
...
function fetchClientsCount() {
    ...
    // Show loading state
    document.getElementById('totalClients').innerHTML = '<i class="bi bi-hourglass-split"></i>';
    
    xhr.onload = function() {
        if (this.status === 200) {
            try {
                const data = JSON.parse(this.responseText);
                document.getElementById('totalClients').textContent = formatNumber(data.count || 0);
            } catch (e) {
...
function fetchOrdersCount() {
    ...
    // Show loading state
    document.getElementById('totalOrders').innerHTML = '<i class="bi bi-hourglass-split"></i>';
    
    xhr.onload = function() {
        if (this.status === 200) {
            try {
                const data = JSON.parse(this.responseText);
                document.getElementById('totalOrders').textContent = formatNumber(data.count || 0);
            } catch (e) {
...
function fetchMonthlyOrdersData() {
    ...
    console.log("Fetching monthly orders data from:", xhr.responseURL || "/api/order/monthly-stats");
    ...
    chartElement.innerHTML = '<div class="d-flex justify-content-center align-items-center" style="height:250px"><span class="spinner-border text-primary" role="status"></span><span class="ms-2">Loading monthly order data...</span></div>';
            ...
            try {
                ...
                setTimeout(() => {
                    try {
                        ...
                        const sampleData = {
                            "Jan": 10, "Feb": 15, "Mar": 20, "Apr": 25,
                            "May": 30, "Jun": 25, "Jul": 20, "Aug": 15,
                            "Sep": 10, "Oct": 15, "Nov": 20, "Dec": 25
                        };
    ...
    xhr.ontimeout = function() {
        ...
        chartElement.innerHTML = '<div class="alert alert-warning">Request timed out. Using sample data instead.</div>';
...
function fetchShopCategoriesData() {
    ...
    // Log the XHR request for debugging
    console.log("Fetching shop categories data from:", xhr.responseURL || "/categorieswithsubcategories");
    ...
    chartElement.innerHTML = '<div class="d-flex justify-content-center align-items-center" style="height:250px"><span class="spinner-border text-primary" role="status"></span><span class="ms-2">Loading category data...</span></div>';
    ...
    xhr.onload = function() {
        ...
        if (this.status === 200) {
            try {
                ...
                setTimeout(() => {
                    try {
                        ...
                        console.log("Attempting to fetch categories from standard categories endpoint...");
                                    ...
                                    if (categoryData && Array.isArray(categoryData) && categoryData.length > 0) {
                                        ...
                                        categoryData.forEach(cat => {
                                            if (cat.name) {
                                                ...
                                                transformedData[cat.name] = Math.floor(Math.random() * 10) + 1;
                                            }
                                        });
            ...
            chartElement.innerHTML = '<div class="alert alert-warning">Attempting to fetch alternative data...</div>';
...
function fetchOrdersPerShop() {
    ...
    // Log the XHR request for debugging
    console.log("Fetching orders per shop from:", xhr.responseURL || "/api/order/by-shop");
    ...
    tableBody.innerHTML = '<tr><td colspan="3" class="text-center"><span class="spinner-border spinner-border-sm text-primary" role="status"></span> Loading shop data...</td></tr>';
    ...
    xhr.onload = function() {
        ...
        if (this.status === 200) {
            try {
                ...
                if (!data || !Array.isArray(data) || data.length === 0) {
                    console.warn("Empty or invalid orders per shop data, creating placeholder data");
    ...
    xhr.onerror = function() {
        ...
        tableBody.innerHTML = '<tr><td colspan="3" class="text-center"><div class="alert alert-danger m-0 py-2">Failed to load shop data. Will attempt to fetch basic shop information.</div></td></tr>';
    ...
    xhr.ontimeout = function() {
        ...
        tableBody.innerHTML = '<tr><td colspan="3" class="text-center"><div class="alert alert-warning m-0 py-2">Request timed out. Generating placeholder data.</div></td></tr>';
...
function fetchBestSellers() {
    ...
    chartElement.innerHTML = '<div class="d-flex justify-content-center align-items-center" style="height:250px"><span class="spinner-border text-primary" role="status"></span><span class="ms-2">Loading best sellers data...</span></div>';
    
    console.log("Fetching best sellers data from:", xhr.responseURL || "/api/shop/best-sellers");
    ...
    xhr.onerror = function() {
        ...
        chartElement.innerHTML = '<div class="alert alert-danger">Failed to load chart data. Will attempt to generate placeholder data.</div>';
    ...
    xhr.ontimeout = function() {
        ...
        chartElement.innerHTML = '<div class="alert alert-warning">Request timed out. Using placeholder data instead.</div>';
                    ...
                    options: {
                        ...
                        legend: {
                            ...
                            formatter: function(seriesName, opts) {
                                ...
                                const shortName = seriesName.length > 12 ? 
...
function createShopCategoriesChart(categoriesData) {
    try {
            ...
            try {
                ...
                if (Array.isArray(categoriesData)) {
                    ...
                    if (categoriesData.length > 0) {
                        ...
                        if (categoriesData[0].categoryName !== undefined) {
                            ...
                            categories = categoriesData.map(item => item.categoryName || 'Unknown');
                            
                            // Store category IDs for reference when clicking
                            categoryIds = categoriesData.map(item => item._id || 'unknown-id');
                        ...
                        else if (categoriesData[0].name !== undefined && categoriesData[0].count !== undefined) {
                        ...
                        else if (categoriesData[0].category !== undefined && categoriesData[0].count !== undefined) {
                            categories = categoriesData.map(item => item.category || 'Unknown');
                            counts = categoriesData.map(item => item.count || 0);
                            categoryIds = categoriesData.map((_, idx) => `id-${idx}`);
                        }
                ...
                if (categories.length > 1 && categories[0] !== 'No Data Available') {
                    ...
                    if (categories.length > 8) {
                        const othersCount = counts.slice(7).reduce((sum, count) => sum + count, 0);
        ...
        if (window.currentChartView === 'subcategories' && window.selectedCategory) {
                ...
                events: {
                    dataPointSelection: function(event, chartContext, config) {
                        ...
                        if (window.currentChartView === 'categories' && 
                            ...
                            console.log("Category clicked:", selectedCategoryName, selectedCategoryId);
                            
                            // Find the full category data
                            const selectedCategoryData = window.fullCategoriesData.find(cat => 
                                cat._id === selectedCategoryId || cat.categoryName === selectedCategoryName
                            );
        ...
        colors: colorPalette,
            ...
            legend: {
                ...
                formatter: function(seriesName, opts) {
                    ...
                    const total = opts.w.globals.series.reduce((sum, val) => sum + val, 0);
    ...
    try {
        ...
        if (sellersData && sellersData.length) {
            try {
                ...
                if (sellersData[0].name !== undefined && sellersData[0].sales !== undefined) {
                ...
                else if (sellersData[0].shopName !== undefined && sellersData[0].orderCount !== undefined) {
                ...
                else if (sellersData[0].shopName !== undefined && sellersData[0].sales !== undefined) {
                    ...
                    parsedData = sellersData.map((item, index) => {
                        if (typeof item === 'object') {
                            ...
                            let name = item.name || item.shopName || item.title || 'Shop ' + (index + 1);
                            let value = item.sales || item.orderCount || item.orders || item.count || 0;
...
function createSubcategoriesChart(categoryData) {
    try {
        ...
        const subcategories = categoryData.subCategories.map(sub => sub.name || 'Unknown');
        ...
        const subcategoryIds = categoryData.subCategories.map(sub => sub._id || 'unknown-id');
        ...
        console.log("Subcategories chart data:", {subcategories, productCounts, subcategoryIds});
        ...
        const options = {
            ...
            colors: colorPalette,            plotOptions: {
                pie: {
                    expandOnClick: false,
                    donut: {
                        size: '55%',
                    ...
                    options: {
                        ...
                        legend: {
                            ...
                            formatter: function(seriesName, opts) {
                                ...
                                const shortName = seriesName.length > 15 ? 
...
function setupCategoryChartUI() {
    setTimeout(() => {
        try {
            ...
            document.querySelectorAll('.apexcharts-pie-series path, .apexcharts-donut-series path').forEach(path => {
            ...
            if (window.currentChartView === 'categories') {
                const chartContainer = document.getElementById('shopCategoriesChartContainer');
                if (chartContainer && !document.getElementById('categoryInfoTip')) {
                    ...
                    infoTip.innerHTML = '<i class="bi bi-info-circle"></i> Click on a category to see subcategories';
            ...
            const chartElement = document.getElementById('shopCategoriesChartContainer');
            if (chartElement) {
                chartElement.style.opacity = '0';
                chartElement.style.transform = 'translateY(10px)';
                chartElement.style.transition = 'opacity 0.5s ease, transform 0.5s ease';
...
function enhanceSubcategoryChartStyle() {
    ...
    setTimeout(() => {
        try {
            ...
            document.querySelectorAll('.apexcharts-datalabel-label').forEach(label => {
                if (label.textContent === 'Total Products' || label.textContent === 'Total') {
            ...
            document.querySelectorAll('.apexcharts-datalabel-value').forEach(value => {
            ...
            if (titleElement && window.currentChartView === 'subcategories') {
                ...
                titleElement.style.background = 'linear-gradient(90deg, #333, #666, #333)';
    ...
    if (shopsData && shopsData.length > 0) {
        try {
            ...
            const normalizedData = shopsData.map(shop => {
                ...
                shopName = shop.name || shop.shopName || shop.shop_name || 'Unknown Shop';
                
                // Handle different order count properties
                orderCount = shop.orderCount || shop.orders || shop.count || 0;
                if (typeof orderCount !== 'number') orderCount = parseInt(orderCount) || 0;
            ...
            const row = document.createElement('tr');
            row.innerHTML = '<td colspan="3" class="text-center">Error processing data</td>';
            tableBody.appendChild(row);
        }
    } else {
        const row = document.createElement('tr');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM content loaded event fired - verifying dashboard initialization");
    ...
    if (typeof window.dashboardInitialized === 'undefined') {
        ...
        const initDashboardIfReady = function() {
            if (typeof ApexCharts !== 'undefined' && !window.dashboardInitialized) {
                console.log("ApexCharts is available and dashboard not initialized - initializing now");
