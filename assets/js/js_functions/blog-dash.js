...
function initEventListeners() {
  ...
  const categoryFilter = document.querySelector(".blog-filter select:nth-child(1)")
  const statusFilter = document.querySelector(".blog-filter select:nth-child(2)")
  ...
  const draftStatsCard = document.querySelector(".stat-card:nth-of-type(4)")
  if (draftStatsCard) {
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
        ...
        const defaultCategories = ["Technology", "Fashion", "Health", "Food", "Travel", "Lifestyle"]
      ...
      console.error("Erreur lors du chargement des catégories:", xhr.statusText)
...
function updateBlogStats() {
  // Total des blogs
  const totalBlogsElement = document.querySelector(".stat-info h3:nth-of-type(1)")
  ...
  const viewsElement = document.querySelector(".stat-info h3:nth-of-type(2)")
  if (viewsElement) {
    viewsElement.textContent = totalViews > 1000 ? (totalViews / 1000).toFixed(1) + "K" : totalViews
  }
  ...
  const commentsElement = document.querySelector(".stat-info h3:nth-of-type(3)")
  if (commentsElement) {
  ...
  const draftBlogsElement = document.querySelector(".stat-info h3:nth-of-type(4)")
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
        ...
        if (otherImagesPreview && blog.otherImagesBlog && blog.otherImagesBlog.length > 0) {
      ...
      if (result.isConfirmed) {
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            console.error("Erreur lors de la suppression du blog:", xhr.statusText)
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
        ...
        showAlert(isUpdate ? "Blog mis à jour avec succès" : "Blog créé avec succès", "success")
      } catch (error) {
        console.error("Erreur lors du parsing de la réponse:", error)
      ...
      console.error("Erreur lors de la soumission du formulaire:", xhr.statusText)
...
function showAlert(message, type) {
  ...
  if (typeof Swal !== "undefined") {
    ...
    console.error("Swal is not defined. Make sure SweetAlert2 is properly imported.")
