...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetchCategories()
      ...
      if (categories.length > 0) {
        ...
        if (pillsTab) {
          ...
          pillsTab.addEventListener("shown.bs.tab", (event) => {
            ...
            if (activatedTabButton && typeof activatedTabButton.getAttribute === "function") {
              const categoryName = activatedTabButton.getAttribute("data-category-name")
              if (categoryName) {
          ...
          pillsTab.addEventListener("click", (event) => {
            const clickedButton = event.target.closest("button[data-category-name]")
            if (clickedButton) {
              const categoryName = clickedButton.getAttribute("data-category-name")
              if (categoryName) {
                // Forcer l'affichage du contenu de l'onglet
                const tabId =
    ...
    .catch((error) => {
      console.error("Failed to initialize categories and trending products:", error)
      fetchAndPopulateTrendingProducts("all")
    })
})
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status === 200) {
        ...
        if (window.mallifyCounters && typeof window.mallifyCounters.incrementWishlistCount === "function") {
          ...
          const currentCount = Number.parseInt(localStorage.getItem("wishlistCount") || "0")
...
function populateCategoryTabs(categories) {
  ...
  categories.slice(0, maxVisibleCategories).forEach((category) => {
    if (category && category.categoryName) {
      ...
      const categoryIdSanitized = category.categoryName.replace(/\s+/g, "-").replace(/&/g, "and") // Sanitize name for ID
      tabItem.innerHTML = `
      ...
      loadMoreButton.addEventListener("click", () => {
        ...
        categories.slice(maxVisibleCategories).forEach((category) => {
          if (category && category.categoryName) {
            ...
            tabItem.className = "nav-item"
            tabItem.setAttribute("role", "presentation")
            const categoryIdSanitized = category.categoryName.replace(/\s+/g, "-").replace(/&/g, "and")
            tabItem.innerHTML = `
                        <button class="nav-link fw-medium text-sm hover-border-main-600" id="pills-${categoryIdSanitized}-tab" data-bs-toggle="pill" data-bs-target="#pills-${categoryIdSanitized}" type="button" role="tab" aria-controls="pills-${categoryIdSanitized}" aria-selected="false" data-category-name="${category.categoryName}">${category.categoryName}</button>
                    `
            pillsTab.appendChild(tabItem)
          } else {
            console.warn("Skipping category in 'More' due to missing categoryName:", category)
          }
        })
...
function createProductCardTrending(product) {
  ...
  const reviews = product.reviewCount || 0
  const price = product.productPrice || "N/A"
  const originalPrice = product.originalPrice || Math.round(Number.parseFloat(price) * 1.25) // Assuming originalPrice might not always be present
  const name = product.productName || "N/A"
  const id = product._id
  const shopName = product.shop?.shopName || "Unknown Shop"
...
function fetchAndPopulateTrendingProducts(category = "all") {
  ...
  const sanitizedCategoryForId = category.replace(/\s+/g, "-").replace(/&/g, "and")
  ...
  if (!currentTabPane) {
    ...
    currentTabPane.setAttribute("aria-labelledby", `pills-${sanitizedCategoryForId}-tab`)
  ...
  if (!productsContainer) {
    console.error("Products container .row.g-12 not found in tab pane:", targetPaneId)
    return
  }
    ...
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        try {
          ...
          console.error(`Error parsing trending products JSON for ${category}:`, error, xhr.responseText)
          productsContainer.innerHTML = `
            <div class="col-12 text-center py-5">
        ...
        console.error(`Could not fetch trending products for ${category}: HTTP error! status: ${xhr.status}`)
        productsContainer.innerHTML = `
          <div class="col-12 text-center py-5">
  ...
  xhr.onerror = () => {
    console.error(`Could not fetch trending products for ${category}: Network error`)
    productsContainer.innerHTML = `
      <div class="col-12 text-center py-5">
        <div class="alert alert-danger">
