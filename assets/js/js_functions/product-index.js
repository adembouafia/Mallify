...
// Fonction principale de catégorisation des produits
function categorizeProducts(products) {
  ...
  // Copie des produits pour éviter de modifier l'original
  ...
  // 1. DEAL OF THE WEEK - Produits avec le meilleur rapport qualité/prix
  // Critères: Produits avec une bonne note ET un prix réduit
  result.dealOfTheWeek = allProducts
    .filter(product => {
      // Calculer un score basé sur la réduction de prix et la note
      const originalPrice = product.originalPrice || Math.round(product.productPrice * 1.2);
      const discountPercentage = ((originalPrice - product.productPrice) / originalPrice) * 100;
      const rating = product.averageRating || 0;
      
      // Produits avec au moins 15% de réduction et une note d'au moins 3.5
      return discountPercentage >= 15 && rating >= 3.5 && product.availability === "In stock";
    })
    .sort((a, b) => {
      // Trier par pourcentage de réduction (décroissant)
      const discountA = ((a.originalPrice || Math.round(a.productPrice * 1.2)) - a.productPrice) / 
                        (a.originalPrice || Math.round(a.productPrice * 1.2));
      const discountB = ((b.originalPrice || Math.round(b.productPrice * 1.2)) - b.productPrice) / 
                        (b.originalPrice || Math.round(b.productPrice * 1.2));
      return discountB - discountA;
    })
    .slice(0, 8); // Limiter à 8 produits

  // 2. TOP SELLING - Produits les plus vendus
  // Critères: Basé sur le nombre d'avis (comme indicateur de popularité)
  result.topSelling = allProducts
    .filter(product => product.availability === "In stock")
    .sort((a, b) => (b.reviewCount || 0) - (a.reviewCount || 0))
    .slice(0, 5);

  // 3. TRENDING - Produits tendance
  // Critères: Produits récents avec une bonne note
  result.trending = allProducts
    .filter(product => {
      // Vérifier si le produit est récent (moins de 30 jours)
      const createdAt = new Date(product.createdAt || now);
      const daysSinceCreation = Math.floor((now - createdAt) / (1000 * 60 * 60 * 24));
      return daysSinceCreation <= 30 && (product.averageRating || 0) >= 3 && product.availability === "In stock";
    })
    .sort((a, b) => {
      // Trier d'abord par date (plus récent en premier)
      const dateA = new Date(a.createdAt || now);
      const dateB = new Date(b.createdAt || now);
      return dateB - dateA;
    })
    .slice(0, 12);

  // 4. FEATURED - Produits mis en avant
  // Critères: Produits marqués comme "featured" ou avec des caractéristiques exceptionnelles
  result.featured = allProducts
    .filter(product => {
      // Produits avec une note élevée OU un prix élevé (produits premium)
    ...
    .sort((a, b) => {
      // Trier par prix (décroissant) pour mettre en avant les produits premium
      return b.productPrice - a.productPrice;
    })
    .slice(0, 8);

  // 5. RECOMMENDED - Produits recommandés
  // Critères: Produits avec un bon rapport qualité/prix et des avis positifs
  result.recommended = allProducts
    .filter(product => {
      // Produits avec une bonne note et au moins quelques avis
      return (product.averageRating || 0) >= 3.8 && 
             (product.reviewCount || 0) >= 2 && 
             product.availability === "In stock";
    })
    .sort((a, b) => {
      // Score combiné: note * nombre d'avis (pour favoriser les produits bien notés avec beaucoup d'avis)
...
// Fonction principale pour récupérer et catégoriser tous les produits
function fetchAndCategorizeProducts() {
    ...
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        try {
          ...
          populateDealsSection(categorized.dealOfTheWeek);
          populateTopSellingSection(categorized.topSelling);
          populateFeaturedSection(categorized.featured);
          populateRecommendedSection(categorized.recommended);
          
          // Initialiser les catégories et les produits tendance
          fetchCategories()
            .then((categories) => {
                ...
                if (pillsTab) {
                  ...
                  pillsTab.addEventListener("shown.bs.tab", (event) => {
                    ...
                    if (
                      ...
                      if (categoryName) {
                        populateTrendingSection(categorized.trending, categoryName);
                      }
                    }
                  });
                  ...
                  pillsTab.addEventListener("click", (event) => {
                    ...
                    if (clickedButton) {
                      ...
                      if (categoryName) {
                        ...
                        // Charger les produits tendance pour cette catégorie
                        populateTrendingSection(categorized.trending, categoryName);
                      }
                    }
                  });
              ...
              // Initialiser avec tous les produits tendance
              populateTrendingSection(categorized.trending, "all");
            })
            .catch((error) => {
              console.error(
...
// Fonction pour remplir la section "Deal of the Week"
function populateDealsSection(products) {
  ...
  if (window.jQuery && window.jQuery(dealsContainer).hasClass("slick-initialized")) {
    window.jQuery(dealsContainer).slick("unslick");
  }
  ...
  if (window.jQuery && window.jQuery.fn.slick) {
    window.jQuery(dealsContainer).slick({
      slidesToShow: 4,
      slidesToScroll: 1,
      arrows: true,
      prevArrow: window.jQuery("#deal-week-prev"),
      nextArrow: window.jQuery("#deal-week-next"),
      responsive: [
...
// Fonction pour remplir la section "Top Selling"
function populateTopSellingSection(products) {
  const topSellingContainer = document.querySelector("#top-selling-slider");

  if (!topSellingContainer) {
    console.error("Top selling container not found");
    return;
  }

  if (!products || products.length === 0) {
    topSellingContainer.innerHTML = "<p>No Top Selling Products Available</p>";
    return;
  }

  if (window.jQuery && window.jQuery(topSellingContainer).hasClass("slick-initialized")) {
    window.jQuery(topSellingContainer).slick("unslick");
  }
  ...
  if (window.jQuery && window.jQuery.fn.slick) {
    window.jQuery(topSellingContainer).slick({
      slidesToShow: 4,
      slidesToScroll: 1,
      arrows: true,
      prevArrow: window.jQuery("#top-selling-prev"),
      nextArrow: window.jQuery("#top-selling-next"),
      responsive: [
        { breakpoint: 992, settings: { slidesToShow: 2 } },
        { breakpoint: 576, settings: { slidesToShow: 1 } },
      ],
...
// Fonction pour remplir la section "Featured Products"
function populateFeaturedSection(products) {
  const featuredContainer = document.querySelector(".featured-product-slider");
  ...
  if (window.jQuery && window.jQuery(featuredContainer).hasClass("slick-initialized")) {
    window.jQuery(featuredContainer).slick("unslick");
  }
  ...
  if (window.jQuery && window.jQuery.fn.slick) {
    window.jQuery(featuredContainer).slick({
      slidesToShow: 1,
      slidesToScroll: 1,
      arrows: true,
      prevArrow: window.jQuery("#featured-products-prev"),
      nextArrow: window.jQuery("#featured-products-next"),
    });
  }
...
// Fonction pour remplir la section "Recommended Products"
function populateRecommendedSection(products) {
  ...
  if (!products || products.length === 0) {
    recommendedContainer.innerHTML = "<p>No Recommended Products Available</p>";
    return;
  }
...
// Fonction pour remplir la section "Trending Products" par catégorie
function populateTrendingSection(products, category = "all") {
  ...
  if (category !== "all") {
    // Simuler le filtrage par catégorie (à remplacer par votre logique réelle)
    // Dans un cas réel, vous feriez une requête API ou filtreriez les produits par catégorie
    // Ici, nous utilisons un filtrage aléatoire pour la démonstration
    filteredProducts = products.filter(() => Math.random() > 0.3);
  }
...
function createProductCard(product) {
  ...
  wrapper.innerHTML = `
    <div class="product-card h-100 p-16 border border-gray-100 hover-border-main-600 rounded-16 position-relative transition-2" data-product-id="${id}">
        ...
        ${discountPercentage >= 15 ? `<span class="product-card__badge bg-success-600 px-8 py-4 text-sm text-white position-absolute inset-inline-start-0 inset-block-start-0">-${discountPercentage}%</span>` : ''}
...
function createCardRecommendedProduct(produit) {
  ...
  const reviews = produit.reviewCount || 0;
  const price = produit.productPrice || "N/A";
  const originalPrice = produit.originalPrice || Math.round(price * 1.3); // Calculer un prix original 30% plus élevé
  const name = produit.productName || "N/A";
  const id = produit._id;
...
document.addEventListener("DOMContentLoaded", () => {
  // Initialiser la catégorisation des produits
...
// Ajouter des styles CSS pour les boutons de favoris
