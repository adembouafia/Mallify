document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupEventListeners() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target.matches('.shop-sidebar ul li a') || 
                e.target.closest('.shop-sidebar ul li a')) {
                e.preventDefault();
    ...
    function generateStarRating(rating) {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= roundedRating) {
                ...
                starsHtml += '<span class="text-15 fw-medium text-warning-600 d-flex"><i class="ph-fill ph-star-half"></i></span>';
            } else {
    ...
    function createProductCard(product) {
        ...
        cardDiv.className = 'product-card h-100 p-16 border border-gray-100 hover-border-main-600 rounded-16 position-relative transition-2';
    ...
    function updateResultCount() {
        const resultCountEl = document.querySelector('.vendor-two-details__contents .d-flex.align-items-center span.text-gray-900');
        if (resultCountEl) {
