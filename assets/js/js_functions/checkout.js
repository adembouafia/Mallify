document.addEventListener('DOMContentLoaded', function() {
  ...
  function populateFormFields(clientInfo) {
    ...
    let firstNamePopulated = populateField('#firstname', clientInfo.firstname || clientInfo.firstName);
    let lastNamePopulated = populateField('#lastname', clientInfo.lastname || clientInfo.lastName);
    let emailPopulated = populateField('#email', clientInfo.email);
  ...
  function setupPlaceOrderButton(clientId, token) {
    const placeOrderButton = document.querySelector('#placeOrderBtn') || document.querySelector('.checkout-sidebar .btn-main');
    if (!placeOrderButton) {
    ...
    xhr.onload = function() {
      ...
      if (xhr.status === 200 || xhr.status === 201) {
        try {
          ...
          if (response.order && response.invoice) {
            ...
            if (typeof Swal !== 'undefined') {
              ...
              alert('Échec de la commande: ' + (response.message || 'Un problème est survenu lors du traitement de votre commande.'));
            }
          }
        } catch (error) {
