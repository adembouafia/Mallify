    ...
    <div class="product-card__content mt-16">
      ...
      <button 
        class="product-card__cart btn ${
          product.stock > 0 ? "bg-gray-50 text-heading hover-bg-main-600 hover-text-white" : "bg-gray-100 text-gray-500"
...
function setupProductCardEventListeners(productCard, productId, productStock) {
  ...
  if (wishlistBtn) {
    ...
    wishlistBtn.addEventListener("click", function (event) {
      ...
      this.classList.contains("active") ? removeFromWishlist(productId) : addToWishlist(productId)
    })
  }
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      if (window.mallifyCounters && typeof window.mallifyCounters.incrementWishlistCount === "function") {
        ...
        if (wishlistCountElement) {
          const currentCount = Number.parseInt(wishlistCountElement.textContent) || 0
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
        ...
        const isInWishlist = favorites.some((item) => {
          ...
          return itemId === productId || (itemId && itemId._id === productId) || (item && item.id === productId)
        })
...
function applyFiltersFromURL() {
  ...
  if (searchParam || nameParam) {
    ...
    const searchInputs = document.querySelectorAll('input.search-form__input, input[type="text"], .form-control')
...
function applyAllFilters() {
  ...
  if (activeCategoryLink && activeCategoryLink.dataset.category !== "all" && activeCategoryLink.dataset.id) {
  ...
  if (searchInput && searchInput.value.trim() && !filters.name && !filters.search) {
      ...
      items += `
        <li class="page-item ${i === currentPage ? "active" : ""}">
          <a class="page-link h-64 w-64 flex-center text-md rounded-8 fw-medium ${
            i === currentPage ? "bg-main-600 text-white" : "text-neutral-600 border border-gray-100"
          }" 
...
function setupCategoryDropdown() {
  ...
  categoryDropdowns.forEach((dropdown) => {
    ...
    if (dropdown.closest("form.form-location-wrapper, form.search-form, form.search-box")) {
      console.log("Skipping dropdown inside search form - will be handled by form submit")
      return
    }
    ...
    dropdown.addEventListener("change", function (e) {
      ...
      if (
        ...
        (selectedOption && selectedOption.textContent.includes("All Categories"))
      ) {
  ...
  if (typeof jQuery !== "undefined") {
    jQuery(document).ready(($) => {
      $(".js-example-basic-single").on("select2:select", function (e) {
        ...
        if ($(this).closest("form.form-location-wrapper, form.search-form, form.search-box").length > 0) {
...
function initSearchForms() {
  ...
  const searchButtons = document.querySelectorAll(".search-form__button, button[type='submit']")
  ...
  searchButtons.forEach((button) => {
    ...
    button.addEventListener("click", (e) => {
      ...
      const searchInput = document.querySelector('input[type="text"], .form-control, .search-form__input')
