document.addEventListener("DOMContentLoaded", function () {
  ...
  const mobileLangItem = document.querySelector(
    ".on-hover-item.has-submenu.arrow-white"
  );
      ...
      link.addEventListener("click", function (e) {
        ...
        if (mobileLangText) {
          mobileLangText.textContent =
            langCode.charAt(0).toUpperCase() + langCode.slice(1, 3);
        }
      });
    });
  }
  ...
  if (mobileLangList) {
    mobileLangList.querySelectorAll("a").forEach((link) => {
      link.addEventListener("click", function (e) {
        ...
        else if (langText.includes("Arabic")) langCode = "ar";
  ...
  function loadGoogleTranslate() {
    if (!window.google || !window.google.translate) {
      ...
      script.onload = function () {
        ...
        // If there's a saved language preference, apply it now
  ...
  function initializeLanguage() {
    ...
    const savedLang =
      urlLang || localStorage.getItem("selectedLanguage") || "en";
    ...
    if (mobileLangText) {
      mobileLangText.textContent =
        savedLang.charAt(0).toUpperCase() + savedLang.slice(1, 3);
    }

    // Try to set the language after a delay to give Google Translate time to load
    setTimeout(() => {
      if (window.google && window.google.translate) {
        changeLanguage(savedLang);
      } else {
        // If Google Translate is still not loaded, store the preference for later
