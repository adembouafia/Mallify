// Use a function that checks for the global superadmin toast or falls back to local implementation
...
function deleteShop(shopId) {
  ...
  if (token) {
    ...
    shopToast("Authentication error: Please log in", "danger");
    return;
  }
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      let errorMessage = "Error deleting shop";
      try {
        if (xhr.responseText) {
          const response = JSON.parse(xhr.responseText);
          errorMessage = response.message || errorMessage;
      ...
      if (xhr.status === 401) {
        shopToast("Unauthorized: Please log in as admin", "danger");
      } else if (xhr.status === 403) {
        shopToast("Forbidden: Admin rights required", "danger");
      } else {
        shopToast(errorMessage, "danger");
      }
    }
...
function filterShops(searchTerm) {
  ...
  shops.approved = filteredShops.filter(
    (shop) => shop.status === "Approved" || shop.status === "Banned"
  );
      ...
      searchInput.addEventListener("keyup", function (e) {
        if (e.key === "Enter") {
          ...
          if (searchTerm) {
            ...
            fetchShops(); // Reset to original data
          }
        }
      });
