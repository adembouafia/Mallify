        ...
        try {
          ...
          if (response.subCategories && Array.isArray(response.subCategories)) {
            ...
            const currentCategory =
              document.getElementById("productCategory")?.value || document.getElementById("editProductCategory")?.value
...
function getCategoryForProduct(productId, subCategoryId) {
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
          ...
          if (categoryName) {
            ...
            productRows.forEach((row) => {
              ...
              if (categoryCell) {
                ...
                console.log(`Updated category for product ${productId} to ${categoryName}`)
              } else {
                console.warn(`Category cell not found for product ${productId}`)
              }
            })
          } else {
      ...
      console.error(`Failed to fetch product category for ${productId}:`, xhr.status, xhr.statusText)
    }
...
function initializeSorting() {
  ...
  // Find the parent container for the search and add the sort dropdown before the search
  ...
  sortDropdown.querySelectorAll(".sort-option").forEach((option) => {
    option.addEventListener("click", (e) => {
      ...
      const index = sortableColumns.findIndex((col) => col.field === currentSortField)
  ...
  // Add event listener to the entries select
  const entriesSelect = entriesPerPageContainer.querySelector(".entries-select")
  entriesSelect.addEventListener("change", function () {
    itemsPerPage = Number.parseInt(this.value)
    currentPage = 1 // Reset to first page when changing items per page
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        if (newSubcategory) {
          ...
          if (parentCategory) {
            ...
            showToast("success", response.message || "Subcategory added successfully!")
          } else {
