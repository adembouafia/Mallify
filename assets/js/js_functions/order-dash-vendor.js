    ...
    if (result.isConfirmed) {
      ...
      xhr.onload = () => {
        if (xhr.status === 200) {
          ...
          console.error("Erreur lors de la reprogrammation de la commande:", xhr.status)
          Swal.fire({
            title: "Erreur!",
      ...
      xhr.onerror = () => {
        console.error("Erreur réseau lors de la reprogrammation de la commande")
        Swal.fire({
          title: "Erreur réseau",
          text: "Veuillez vérifier votre connexion et réessayer.",
          icon: "error",
    ...
    if (result.isConfirmed) {
      ...
      xhr.onload = () => {
        if (xhr.status === 200) {
          ...
          try {
            const response = JSON.parse(xhr.responseText)
            showErrorMessage(response.message || "Erreur lors de l'expédition de la commande")
          } catch (e) {
