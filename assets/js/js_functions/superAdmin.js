// Create a namespace for SuperAdmin functionality
const SuperAdmin = (function() {
    // Private variables
    let logoutListenerInitialized = false;
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalItems = 0;
    let allAdmins = [];
    ...
    function updateUserMenuInfo() {
        try {
            ...
            // Use a flag to ensure we only set up the logout listener once
            if (!logoutListenerInitialized) {
                ...
                logoutListenerInitialized = true;
            }
        } catch (error) {
    ...
    // Public API - functions and variables exposed to the global scope
    return {
        init: init,
        showToast: showToast,
...
// Initialize SuperAdmin when DOM is loaded
document.addEventListener('DOMContentLoaded', SuperAdmin.init);

// Expose the toast function globally so other scripts can use it
window.showToast = SuperAdmin.showToast;