...
function displayProductDetails(product) {
  ...
  const skuElement = document.querySelector(
    ".flex-align.flex-wrap.gap-12 + span + span"
  );
  if (skuElement) {
    skuElement.innerHTML = `<span class="text-gray-400">REF:</span>${product.productId || "N/A"}`;
  }
  ...
  const discountElement = document.querySelector(
    ".flex-align.gap-8.text-main-two-600"
  );
  if (discountElement && product.regularPrice && product.productPrice) {
    const discount = Math.round(
      ((product.regularPrice - product.productPrice) / product.regularPrice) *
        100
    );
    if (discount > 0) {
  ...
  const descriptionElement = document.querySelector(
    ".product-details__content p.text-gray-700"
  );
  ...
  if (descTabContent) {
    const detailedDescription = document.querySelector(
      "#pills-description p:first-of-type"
    );
    if (detailedDescription) {
      detailedDescription.innerHTML =
        product.productDetails || "No detailed description available";
    }
  }
  ...
  let availabilityElement = document.getElementById("product-availability");
  const availability =
  ...
  if (
    availability.toLowerCase().includes("out") ||
    ...
    availability.toLowerCase().includes("limited") ||
    product.stock < 5
  ) {
    iconClass = "ph-clock";
  ...
  if (!availabilityElement) {
    ...
    if (stockContainer) {
      ...
      availabilityElement.id = "product-availability";
      availabilityElement.className = "mt-16 d-flex align-items-center";
      availabilityElement.innerHTML = `
                <span class="w-32 h-32 ${bgColor} ${iconColor} rounded-circle flex-center text-xl flex-shrink-0 me-3">
                    <i class="ph-fill ${iconClass}"></i>
                </span>
                <div>
                    <span class="text-sm text-neutral-600">Availability: </span>
                    <span class="text-sm fw-semibold availability-status">${availability}</span>
                </div>`;
      stockContainer.appendChild(availabilityElement);
    }
  } else {
    // Update existing availability element
    const iconElement = availabilityElement.querySelector("i");
    const iconContainer = availabilityElement.querySelector("span.w-32");
    const statusElement = availabilityElement.querySelector(
      ".availability-status"
    );

    if (iconElement) {
      iconElement.className = `ph-fill ${iconClass}`;
    }

    if (iconContainer) {
      iconContainer.className = `w-32 h-32 ${bgColor} ${iconColor} rounded-circle flex-center text-xl flex-shrink-0 me-3`;
    }
      ...
      try {
        ...
        if (
          ...
          shopNameElement.textContent =
            response.data.shop.shopName || "Unknown Shop";
        } else {
          shopNameElement.textContent = "Shop";
      ...
      try {
        ...
        if (
          ...
          if (response.data.reviews.length > 0) {
            fetchClientProfiles(response.data.reviews).then(
              (enrichedReviews) => {
                ...
                const stats = {
                  averageRating:
                    ...
                    calculateAverageRating(enrichedReviews),
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < enrichedReviews.length; i++) {
      ...
      if (review.clientId) {
        ...
        xhr.open(
          "GET",
          `http://localhost:3000/client/${review.clientId}`,
          true
        );
        ...
        xhr.onload = function () {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            try {
              ...
              console.warn(
                "Could not parse client data for",
                review.clientId,
                err
              );
            }
          ...
          if (completedRequests === reviewsWithClientId.length) {
            console.log(
              "Enriched reviews with profile pictures:",
              enrichedReviews
            );
...
function displayReviews(reviews) {
  const container = document.querySelector(
    "#pills-reviews .col-lg-6:first-of-type"
  );
  ...
  reviews.forEach((review) => {
    ...
    el.className =
      "d-flex align-items-start gap-24 pb-44 border-bottom border-gray-100 mb-44 review-item";
  ...
  const stats = {
    averageRating:
      currentProduct.averageRating || calculateAverageRating(reviews),
...
function updateReviewStats(stats) {
  ...
  const avgRatingElement = document.querySelector(
    ".border.border-gray-100.rounded-8.px-40.py-52 h2"
  );
  if (avgRatingElement) {
  ...
  const ratingContainers = document.querySelectorAll(
    ".flex-align.gap-8.mb-20, .flex-align.gap-8.mb-0"
  );
  ...
  const starsContainer = document.querySelector(
    ".border.border-gray-100.rounded-8.px-40.py-52 .flex-align.gap-8"
  );
  if (starsContainer) {
  ...
  for (let i = 0; i < ratingContainers.length; i++) {
    ...
    const countElement = container.querySelector(
      ".text-gray-900.flex-shrink-0:last-child"
    );
    ...
    if (progressBar && countElement) {
      ...
      console.log(
        `Rating ${rating}: ${count} reviews, ${percentage.toFixed(1)}%`
      );
...
function handleReviewSubmit(event) {
  ...
  if (!titleInput.value)
    return showAlert("danger", "Please enter a review title");
  if (!contentInput.value)
    return showAlert("danger", "Please enter review content");
  if (!token)
    return showAlert("danger", "You must be logged in to submit a review");
...
function setupQuantityButtons() {
  ...
  if (!minusBtnOrig || !plusBtnOrig || !quantityInput || !currentProduct)
    return;
  ...
  quantityInput.value = Math.min(
    Math.max(Number.parseInt(quantityInput.value) || 1, 1),
    maxStock
  );
  ...
  if (addToCartBtn) {
    ...
    newAddToCartBtn.addEventListener("click", (e) => {
      ...
      const quantity = Number.parseInt(
        document.getElementById("side-price-insert-prod").value
      );
...
function calculateAverageRating(reviews) {
  ...
  const total = reviews.reduce(
    (sum, review) => sum + (parseInt(review.rating) || 0),
    0
  );
...
function updateProductStarRating(rating) {
  const starContainer = document.querySelector(
    ".flex-align.flex-wrap.gap-12 .flex-align.gap-8"
  );
  ...
  const ratingText = document.querySelector(
    ".flex-align.flex-wrap.gap-12 .text-sm.fw-medium.text-neutral-600"
  );
  if (ratingText) {
  ...
  const ratingCount = document.querySelector(
    ".flex-align.flex-wrap.gap-12 .text-sm.fw-medium.text-gray-500"
  );
  if (
    ratingCount &&
