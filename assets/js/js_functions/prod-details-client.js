...
function displayProductDetails(product) {
  ...
  const availability = product.availability || (product.stock > 0 ? "In stock" : "Out of stock")
  ...
  if (addToCartBtn) {
    if (availability !== "In stock") {
      ...
      addToCartBtn.title = "This product is currently unavailable"
    } else {
...
function setupAddToCartButton() {
  ...
  if (addToCartBtn) {
    ...
    // Check availability and disable button if not in stock
    if (currentProduct && currentProduct.availability !== "In stock") {
      newAddToCartBtn.disabled = true
      newAddToCartBtn.classList.add("opacity-50")
      newAddToCartBtn.title = "This product is currently unavailable"
    }
    ...
    newAddToCartBtn.addEventListener("click", (e) => {
      ...
      if (currentProduct && currentProduct.availability !== "In stock") {
        showSweetAlert("warning", "Unavailable", "This product is currently out of stock")
        return
      }
...
function calculateAverageRating(reviews) {
  ...
  const total = reviews.reduce((sum, review) => sum + (Number.parseInt(review.rating) || 0), 0)
