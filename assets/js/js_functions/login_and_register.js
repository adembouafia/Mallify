...
// Preview the shop logo
function previewImage(event) {
  const file = event.target.files[0]
  if (file) {
    const reader = new FileReader()
    reader.onload = (e) => {
      const previewDiv = document.getElementById("shopLogoPreview")
      if (previewDiv) {
...
// Preview the vendor image
function previewVendorImage(event) {
  const file = event.target.files[0]
  if (file) {
    const reader = new FileReader()
    reader.onload = (e) => {
      const previewDiv = document.getElementById("vendorImagePreview")
      if (previewDiv) {
        const previewImg = previewDiv.querySelector("img")
        previewImg.src = e.target.result
        previewDiv.style.display = "block"
...
function initializeLoginForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!email || !password) {
      showToast("Required Fields", "All fields are required!", "warning")
      return
    }
    ...
    function tryLogin(index = 0) {
      if (index >= loginEndpoints.length) {
        showToast("Login Failed", "Check your credentials or account type.", "error")
        return
      }
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          try {
            ...
            if (response.status === "Pending") {
              showToast("Shop Pending", "Your shop is being reviewed by administration.", "warning")
              return
            }
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", (e) => {
      ...
      xhr.onload = () => {
        if (xhr.status === 201) {
          const response = JSON.parse(xhr.responseText)
          showToast("Registration Successful!", "Client registered successfully!", "success")
          setTimeout(() => {
          ...
          showToast("Registration Error", error.message || "Registration failed", "error")
          console.error(error)
        }
      }

      xhr.onerror = () => {
        showToast("Connection Error", "Error connecting to the server.", "error")
      }
  ...
  if (vendorForm) {
    vendorForm.addEventListener("submit", (e) => {
      ...
      xhr.onload = () => {
        if (xhr.status === 201) {
          const response = JSON.parse(xhr.responseText)
          showToast("Registration Successful!", "Vendor registered successfully!", "success")
          setTimeout(() => {
            window.location.reload()
            showLogin()
          }, 2000)
...
function validatePassword() {
  ...
  if (newPasswordValue !== confirmPasswordValue || newPasswordValue.length < 6) {
    showToast("Password Validation", "Passwords must match and be at least 6 characters long.", "warning")
    return
  }
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4) {
      try {
        const res = JSON.parse(xhr.responseText)
        if (xhr.status === 200) {
          showToast("Reset Successful", "Your password has been reset successfully!", "success")
          ...
          showToast("Validation Error", res.message || "Invalid or expired code.", "error")
        }
      } catch (error) {
        console.error("Error parsing response:", error)
