  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      tryLogin("admin", loginData)
        ...
        .catch(() => {
          // If admin login fails, try vendor
          return tryLogin("vendor", loginData);
        })
        .then((response) => {
          if (response && response.token) {
            localStorage.setItem("token", response.token);
            localStorage.setItem("userRole", "vendor");
            localStorage.setItem("userData", JSON.stringify(response.vendor));
            window.location.href = "../frontend/dashbordBout_pages/index.html";
          }
        })
        .catch(() => {
          // If vendor login fails, try client
          return tryLogin("client", loginData);
        })
        .then((response) => {
          if (response && response.token) {
            localStorage.setItem("token", response.token);
...
// Helper function to try login for a specific role
function tryLogin(role, loginData) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", `http://localhost:3000/${role}/login`, true);
    ...
    xhr.onerror = () =>
      reject({ message: `Network error during ${role} login` });
    xhr.send(loginData);
  });
}
